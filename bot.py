...
# How many days back to look for posts.  Safe because sent_per_post
# deduplication prevents re-sending.  3 days lets us catch up after downtime.
LOOKBACK_DAYS = 3
...
def is_recent(updated_str, days=LOOKBACK_DAYS):
    """Check if the post was updated within the last N days."""
    if not updated_str:
        return False
    try:
...
def get_recent_matching_posts():
    """
    Fetch matching NBA posts from the last LOOKBACK_DAYS days.
    Uses both the 'new' feed and a search query for better coverage.
    Deduplication by post_id ensures no doubles.
    """
    all_entries = []
    ...
    # Source 2: search feed — catches older posts the /new feed might miss
    time.sleep(2)
    search_q = "NBA+Props+Daily+OR+NBA+Betting+OR+NBA+Picks"
    content2 = fetch_rss(
    ...
    # Deduplicate by post_id and filter
    seen_ids = set()
    matching = []
    for entry in all_entries:
        ...
        print(f"  📌  Recent match: {entry['title'][:60]}")
    return matching
...
- Use "3PM" for three-pointers made stat, even if the user writes "3s" or "3's" or "threes"
...
def main():
    ...
    print(f"  🎯  {len(posts)} matching post(s) found (last {LOOKBACK_DAYS} days)")
    ...
    for post in posts:
        ...
        print(f"\n  📅  Post date from title: {post_date} | {title[:60]}")
        ...
        for author_key, comment in user_comments.items():
            author_display = (comment["author_raw"].strip()
                              .lstrip("/u/").lstrip("u/"))
            print(f"  🎯  Main comment from u/{author_display} "
                  f"({len(comment['body'])} chars)")
            ...
            formatted = format_with_claude(
                author_display, body, title, is_edit=is_edited
            )
            message = build_message(
                title, post_url, author_display, formatted, is_edit=is_edited
            )
            ...
            if is_new:
                bets = parse_bets_with_claude(
                    author_display, body, title, post_date
                )
                state["pending_bets"] = [
